...
\begin{proof}
  ...
  \begin{itemize}
    \item[\ref{ax:ct4}] Consider a map $f\colon H\to H'$ in $\clH$ that admits a pseudo-kernel $f^K\colon H''\to H$ in $\F_2$ such that $H''\in\F_1$ as in the statement of \ref{ax:ct4} and consider the commutative diagram \todo{maybe use $T_1$ instead of $H''[1]$ and explain why they can be used interchangeably}
      \begin{equation*}
        \begin{tikzcd}
...
\end{lemma}
\begin{proof}
  \todo{Change the notation. This was taken straight from the example}
      ...
      \mathrm{Cone}(f)[-1]\arrow{r}{f^K}\arrow{d}{\lambda[-1]}
       ...
       f_2(\mathrm{Cone}(f))[-1]\arrow{r}
        & V_2\arrow{r}
          & H_2\arrow{r}
            &  f_2\mathrm{Cone}(f)\\
        &  t_1(V_2)\arrow{u}{\varepsilon}\arrow[dotted, crossing over, bend left=70]{uu}[near end,']{\beta}
        & &
    \end{tikzcd}
  \end{equation*}
  ...
  gives the existence of $\beta: t_1(V_2)\to H_1$ such that $\alpha\circ\beta=\varepsilon$.

  Since $ t_1$ is a functor, there is also a morphism
  $ t_1(\alpha): t_1(H_1)=H_1\to  t_1(V_2)$ such that
  ...
  $ t_1(\alpha)\circ\beta=1_{ t_1(V_2)}$. Then, $\beta$ is a section.

  Hence, we can write $ t_1(\alpha):H_1\to  t_1(V_2)$
  as
  \begin{equation*}
    \begin{tikzcd}
       t_1(\alpha): t_1(V_2)\oplus H_1'
      \arrow{r}{
          \begin{psmallmatrix}
            \ast \amsamp 0
          \end{psmallmatrix}
  ...
  $V_2\in\T_1$. Hence, $ t_1(V_2)\cong V_2$.
  ...
  $H_1'\in\T_1$ because $H_1\in\T_1$. Hence, $H_1'=0$ and $\alpha:H_1\to V_2$ is an iso.
