...
\begin{proof}
  ...
  \begin{enumerate}
    ...
    \item[$\Rightarrow$] By \ref{prop:1.4}\ref{prop:1.4:b} \todo{fix this reference} there exists $f^C:H_2\to T_2$, where
...
\begin{proof}
  ($(a)\Rightarrow (b1),(b2)$) Assume that $\clH$ is an abelian category. By \ref{cor:2.3}\ref{cor:2.3:d}, $(b1)$ holds if and only if any monomorphism in $\clH$ is a monomorphism in $\A$.

  Observe that for any $f:H\to H'$, we have that $\Ker_\clH(f)\nto{\tilde{f}^H}H$ is a monomorphism in $\A$. Indeed, by \ref{cor:2.3}\ref{cor:2.3:b}, we know that $\tilde{f}^H$ is the composition of
  \begin{equation*}
    \begin{tikzcd}
  ...
  Furthermore, if $f:H\to H'$ is a monomorphism in $\clH$, then $f=\Ker_\clH(\Coker_\clH(f))$ since $\clH$ is abelian. Thus, $f$ is a monomorphism in $\A$.

  A dual argument can be used to prove $(b2)$.

  ($(a)\Rightarrow (b3)$) Let $f:H\to H'$ in $\clH$ be a monomorphism in $\A$. We want to prove that $\Coker(f)\in\clH$.
  ...
  Thus, $\Coker(\alpha)\in\T_2\cap\F_1=0$ and so $\alpha$ is an isomorphism. By \eqref{thm_2_4_eq_1} is follows that $\Coker(f)\cong f_2(\Coker(f))\in\F_2$ and, using the fact that $T_1$ is closed under quotients, we conclude that $\Coker(f)\in\T_1\cap\F_2=\clH$. A dual argument shows that $\clH$ is closed under kernels of epimorphism in $\A$.
  ...
  Let $f:H\to H'$ be a monomorphism in $\clH$. By \ref{cor:2.3} and $(b1)$, it follows that $f$ is a monomorphism in $\A$. Thus, we have a short exact sequence in $\A$
  \begin{equation*}
    \begin{tikzcd}
      0 \arrow{r}
      & H\arrow{r}{f}
  ...
  \begin{equation*}
    ...
    \end{tikzcd}
  \end{equation*}
  where $f'$ is an epi and $f''$ is a mono (in $\clH$). Then, by $(b1)$ and $(b2)$ we have that $f'$ and $f''$ are respectively an epi and a mono in $\A$. Therefore, $\Img(f)\cong \Img_\clH(f)\in\clH$, and by the exact sequence
  \begin{equation*}
    \begin{tikzcd}
      0 \arrow{r}
      & \Img(f)\arrow{r}
...
\begin{definition}
  A pair $(\mathscr{A},\mathcal{B})$ of full subcategories of $\mathcal{T}$ is a t-structure
  in $\mathcal{T}$ if
  \begin{enumerate}[label=(\alph*)]
    \item $\mathbbm{t}=(\mathscr{A},\mathcal{B}[-1])$ is a torsion pair in $\mathcal{T}$, and
    \item $\mathscr{A}[1]\subseteq \mathscr{A}$.
  \end{enumerate}
\end{definition}

\begin{rmk}
  It is well known that any t-structure $(\mathscr{A},\mathcal{B})$ in $\mathcal{T}$
  gives a functional torsion pair $\mathbbm{t}=(\mathscr{A},\mathcal[-1])$ and
  $\mathbb{B}[-1]\subseteq\mathcal{B}$. Furthermore, $\mathscr{A}$ and
  $\mathcal{B}$ are closed under extensions and direct summands. Note that the t-structure
  $(\mathscr{A},\mathcal{B})$ depends only on $\mathscr{A}$, since $\mathcal{B}=\mathscr{A}^\perp[1]$.
\end{rmk}
...
\begin{example}
  Let $(\mathscr{A},\mathcal{B})$ be a t-structure in $\mathcal{T}$. Consider
  $\mathbbm{t}_1:=(\mathscr{A},\mathcal{B}[-1])$ and $\mathbbm{t}_2 :=(\mathscr{A}[1],\mathcal{B})$.
  It is not hard to see that $\mathbbm{t}=(\mathbbm{t}_1,\mathbbm{t}_2)$ is a strongly related
  torsion pair in $\mathcal{T}$. In this case, by \ref{thm:2.6}, we get that $\mathcal{H}=\mathscr{A}\cap\mathcal{B}$
...
Let $t=(\mathscr{A},\mathcal{B})$ be a pair of full subcategories of the triangulated
category $\mathcal{T}$. We will use the following notation:
\begin{align*}
  t[1] &:= (\mathscr{A}[1],\mathcal{B}[1]),\\
  \overline{t} &:= (\mathscr{A},\mathcal{B}[1]).
\end{align*}

Note that $\overline{t}$ is a t-structure in $\mathcal{T}$ if and only if $t$ is a torsion
pair $\mathcal{T}$ such that $\mathscr{A}[1]\subseteq\mathscr{A}$.
...
$\mathscr{A}$ is cohereditary if the class $\mathcal{F}$ is closed under quotients
in $\mathscr{A}$.
