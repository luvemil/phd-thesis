...
\begin{proof}
  \begin{enumerate}[label=(\alph*)]
    ...
    unique $f':T_1\to H$ such that $f'f^C=g$. By Remark \ref{rmk:1.1} \todo{Add a proper reference to part a of the rmk},
    ...
    so $f''\circ \lambda_{2,T_1}=r\circ \lambda_{2,T_1}$ by $\ref{ax:ct3}$, and $f''=r$ by \emph{RMK1.1a}\todo{Add a proper reference}.
    \item Dual.
  \end{enumerate}
\end{proof}
...
\begin{proof} ~
  ...
  \begin{enumerate}
    \item[$(a)\Rightarrow (b)$]
      ...
      by CT4 \todo{Add proper reference} there is a map $b:t_1(F_2)\to H_1$ making the diagram commute.
    ...
    \item[$(b)\Rightarrow (a)$] Let $\mathcal{H}$ be an abelian category. We only check $(CT4)$ \todo{Fix reference}
